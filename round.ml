...
let rec side_bet (deck : deck) (player : player) (dealer : dealer) =
  ...
  print_endline
    "If you do a side bet and win, the amount of chips in parentheses \
     will be added to your chip pile. Please enter '1', '2', or '3' \
...
let rec start_round
    ...
    (cpu : player)
    (lvl : string) =
  print_endline "Would you like to do a side bet? \n";
  print_string "> ";
  let y_n = read_line () in
  match Command.check_yes_no y_n with
  | "yes" ->
